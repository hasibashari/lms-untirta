...
const GetTeacherDashboardStats = async (call, callback) => {
  try {
    ...
    // 1. Identifikasi semua mata kuliah dan kelas yang dikelola
    const [ownedCourses, taughtClasses] = await Promise.all([
      prisma.course.findMany({ where: { teacherId }, select: { id: true } }),
      prisma.class.findMany({ where: { lecturerId: teacherId }, select: { courseId: true, id: true } }),
    ]);

    const courseIds = [...new Set([...ownedCourses.map(c => c.id), ...taughtClasses.map(c => c.courseId)])];
    const classIds = taughtClasses.map(c => c.id);

    // 2. Ambil data agregat per mata kuliah
    const courses = await prisma.course.findMany({
      where: { id: { in: courseIds } },
      include: {
        classes: {
          where: { OR: [{ lecturerId: teacherId }, { course: { teacherId } }] },
          include: { _count: { select: { krsEnrollments: { where: { status: 'APPROVED' } } } } },
        },
    ...
    courses.forEach(course => {
      totalStudents += course.classes.reduce((sum, cls) => sum + (cls._count.krsEnrollments || 0), 0);
