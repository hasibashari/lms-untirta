...
export const create = async (req, res) => {
  try {
    ...
    const result = await grpcCreateClass(req.body, meta);
    sendSuccess(res, {
      statusCode: 201,
...
export const getById = async (req, res) => {
  try {
    const meta = createGrpcMetadata(req);
    const result = await grpcGetClassById({ id: req.params.id }, meta);
    sendSuccess(res, {
      statusCode: 200,
...
export const remove = async (req, res) => {
  try {
    const meta = createGrpcMetadata(req);
    const result = await grpcDeleteClass({ id: req.params.id }, meta);
    sendSuccess(res, {
      statusCode: 200,
