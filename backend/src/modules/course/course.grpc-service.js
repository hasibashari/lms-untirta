...
const GetStudentsByCourse = async (call, callback) => {
  try {
    ...
    // Identifikasi apakah courseId adalah Class ID atau Course ID
    ...
    if (classOffering) {
      ...
      const course = await prisma.course.findUnique({ where: { id: courseId } });
      if (!course) return callback({ code: grpc.status.NOT_FOUND, details: 'Data tidak ditemukan' });
...
const GetAvailableStudentsForCourse = async (call, callback) => {
  try {
    ...
    const students = await prisma.user.findMany({
      where: {
        role: 'MAHASISWA',
        krsEnrollments: { none: { class: { courseId: finalCourseId }, status: 'APPROVED' } },
      },
  ...
  try {
    ...
    const courses = enrollments.map(e => ({
      ...
      course: {
        ...
        materialsCount: e.class.course._count.materials,
        studentsCount: e.class._count.krsEnrollments,
...
const GetTeachingCourses = async (call, callback) => {
  try {
    ...
    const classes = await prisma.class.findMany({
      where: {
        OR: [{ lecturerId: teacherId }, { course: { teacherId: teacherId } }]
      },
      include: {
        course: { include: { _count: { select: { materials: true } } } },
        _count: { select: { krsEnrollments: { where: { status: 'APPROVED' } } } }
      },
      orderBy: { createdAt: 'desc' }
    });
    ...
    const courses = classes.map(cls => ({
      ...
      materialsCount: cls.course._count.materials,
      studentsCount: cls._count.krsEnrollments,
      section: cls.section,
...
const AdminDeleteCourse = async (call, callback) => {
  try {
    ...
    callback(null, { message: 'Mata kuliah berhasil dihapus', deletedStudents: 0 });
  } catch (error) {
    callback({ code: grpc.status.INTERNAL, details: error.message });
  }
};
