 ...
 *
 * Tests the business logic in submission.service.js in isolation.
 * Prisma is fully mocked — no database calls are made.
 *
 * What we test:
 *   ✓ submitAssignment            — not-late / late flag, duplicate prevention, 404
 *   ✓ getAssignmentWithMySubmission — enrollment check, graded/submitted/overdue/pending status
 *   ✓ getSubmissionsByAssignment  — ownership check, returns list
 *   ✓ getAllMyGrades               — flattens enrollments, all status variants, null teacher
 *   ✓ getMyDashboardStats         — zero-case and aggregation from parallel queries
 *   ✓ gradeSubmission             — 2-level ownership, updates correctly
 *   ✓ getTeacherDashboardStats    — zero-case and multi-course aggregation
 *   ✓ getRecentSubmissionsForTeacher — transforms, default limit, isGraded flag
 */
...
const {
  ...
  gradeSubmission,
} = await import('../../src/modules/submission/submission.service.js');
