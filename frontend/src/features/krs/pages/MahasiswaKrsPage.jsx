...
const KartuRencanaStudi = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {/* Tabs Navigation - Premium Dashboard Style */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-1">
        ...
        {/* Action Info Hint */}
        <div className="hidden lg:flex items-center gap-2 text-slate-400 text-xs italic font-medium">
          <Info size={14} className="text-blue-400" />
          {activeTab === 'offered' 
            ? 'Klik tombol "Enroll" untuk menambahkan mata kuliah' 
            : 'Mata kuliah yang sudah Anda pilih muncul di sini'}
        </div>
      </div>
      ...
      <div className="mt-4 relative min-h-[400px]">
        {loading ? (
          <div className="space-y-6 animate-in fade-in duration-500">
            {/* Skeleton for Header/Filter area */}
            <div className="flex gap-4 mb-8">
              <div className="h-10 w-full sm:w-64 bg-slate-200 animate-pulse rounded-lg" />
              <div className="h-10 w-full sm:w-48 bg-slate-200 animate-pulse rounded-lg" />
            </div>
            
            {/* Skeleton for Table/List */}
            <div className="space-y-4">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-20 w-full bg-slate-100 animate-pulse rounded-xl border border-slate-200/50" />
              ))}
            </div>
          </div>
        ) : (
