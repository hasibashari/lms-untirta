...
const LearningSidebar = ({
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 h-screen bg-white border-r border-slate-200 z-50 flex flex-col overflow-hidden
      ...
      >
        ...
        <div className="h-16 border-b border-slate-200 flex items-center justify-between px-4">
          <button
            onClick={onBackClick}
            className={`flex items-center gap-2 text-slate-600 hover:text-blue-600 transition ${collapsed ? 'justify-center' : ''
              }`}
          >
            <ArrowLeft size={18} />
        ...
        <div className={`p-4 border-t border-slate-100 ${collapsed ? 'px-2' : ''}`}>
          <Link
            ...
            className={`flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-slate-100 text-slate-700 font-medium rounded-xl hover:bg-slate-200 transition ${collapsed ? 'px-2' : ''
              }`}
          >
            <Home size={18} />
